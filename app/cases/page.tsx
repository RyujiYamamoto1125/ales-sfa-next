...
export default function CasesPage() {
  ...
  const filtered = cases.filter((c) => {
    ...
    if (filterSales && !c.salesPerson?.includes(filterSales)) return false;
    return true;
  });
  ...
  function startEdit(c: CaseData) {
    ...
    setEditForm({
      ...
      status: c.status,
      nextMeeting: c.nextMeeting ? new Date(c.nextMeeting).toISOString().slice(0, 16) : "",
  ...
  function downloadCSV() {
    ...
    const rows = filtered.map((c) => [
      c.customerName,
      c.status,
      c.nextMeeting ? new Date(c.nextMeeting).toLocaleString("ja-JP") : "",
      c.salesPerson ?? "",
      c.appointer ?? "",
      c.notes ?? "",
        ...
        {tab === "list" && (
          <>
            ...
            <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 mb-4 flex flex-wrap gap-3">
              ...
              <input
                placeholder="アポインター名で絞り込み"
                value={filterAppointer}
                onChange={(e) => setFilterAppointer(e.target.value)}
                className="border border-gray-200 rounded-lg px-3 py-2 text-sm"
              />
              <input
                placeholder="営業担当者名で絞り込み"
                value={filterSales}
            ...
            {loading ? (
              ...
              <div className="space-y-3">
                {filtered.map((c) => (
                  <div key={c._id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                    <div
                      className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-gray-50"
                      onClick={() => setEditId(editId === c._id ? null : c._id)}
                    >
                      ...
                      <div className="flex items-center gap-4 text-sm text-gray-500">
                        {c.salesPerson && <span>営業: {c.salesPerson}</span>}
                        {c.appointer && <span>アポ: {c.appointer}</span>}
                        <span className="text-gray-300">{editId === c._id ? "▲" : "▼"}</span>
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-100 max-w-2xl">
            ...
            <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-xs font-medium text-gray-600 mb-1">アポインター名</label>
                <input
                  value={form.appointer}
                  onChange={(e) => setForm({ ...form, appointer: e.target.value })}
                  className="w-full border border-gray-200 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#26C6DA]"
                />
              </div>
              <div>
